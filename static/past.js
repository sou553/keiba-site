(function () {
  ...
  function renderList() {
    ...
    list.innerHTML = rows.map((obj) => {
      ...
      return `<article class="past-horse-card ${expanded ? 'is-open' : ''}"><button type="button" class="past-horse-card__summary" data-horse-key="${ra.escapeHtml(key)}"><div><div class="past-horse-card__title">${ra.escapeHtml(h.umaban)} ${ra.escapeHtml(h.horse_name)}</div><div class="past-horse-card__meta">近3走 平均着順 ${ra.escapeHtml(ra.fmtNum(obj.avgFinish, 1))} / 掲示板 ${ra.escapeHtml(obj.board3)}回 / 上がり平均 ${ra.escapeHtml(ra.fmtNum(obj.avgLast3f, 1))}</div></div><div class="tag-list"><span class="tag ${obj.sameCourseCount > 0 ? 'tag--plus' : ''}">同コース ${ra.escapeHtml(obj.sameCourseCount)}</span><span class="tag ${obj.sameDistanceCount > 0 ? 'tag--plus' : ''}">同距離 ${ra.escapeHtml(obj.sameDistanceCount)}</span><span class="tag">全${ra.escapeHtml(totalRuns)}</span></div></button><div class="past-horse-card__detail">${runs.length ? runs.map(runCard).join('') : '<div class="section-subtitle">過去走データなし</div>'}<div class="horse-run-actions">${totalRuns > visible ? `<button type="button" class="action-link" data-more-runs="${ra.escapeHtml(key)}">さらに${Math.min(5, totalRuns - visible)}件見る</button>` : ''}${visible > state.limit ? `<button type="button" class="action-link" data-reset-runs="${ra.escapeHtml(key)}">${state.limit}件表示に戻す</button>` : ''}</div></div></article>`;
