(function () {
  ...
  function renderFilters() {
    ...
    root.innerHTML = `
      ...
      <div class="horse-sort-bar">
        ...
        <button type="button" class="sort-chip ${state.sortKey === 'p_win' ? 'is-active' : ''}" data-sort="p_win">勝率順</button>
        <button type="button" class="sort-chip ${state.sortKey === 'hole_score' ? 'is-active' : ''}" data-sort="hole_score">穴スコア順</button>
        <button type="button" class="sort-chip ${state.sortKey === 'danger_score' ? 'is-active' : ''}" data-sort="danger_score">危険スコア順</button>
        <button type="button" class="sort-chip ${state.sortKey === 'agreement_score' ? 'is-active' : ''}" data-sort="agreement_score">一致度順</button>
    ...
    qs('#toggle-sort-dir', root)?.addEventListener('click', () => {
      state.sortDir = state.sortDir === 'asc' ? 'desc' : 'asc';
      renderFilters();
      applyFiltersAndRenderList();
    });
  ...
  function compareHorses(a, b) {
    ...
    switch (state.sortKey) {
      case 'pred_order': av = a._norm.pred_order; bv = b._norm.pred_order; return compareNullable(av, bv, asc);
      case 'course_adv_rank': av = a._norm.course_adv_rank; bv = b._norm.course_adv_rank; return compareNullable(av, bv, asc);
      case 'popularity': av = a._norm.popularity; bv = b._norm.popularity; return compareNullable(av, bv, asc);
      case 'umaban': av = raToNum(a.umaban); bv = raToNum(b.umaban); return compareNullable(av, bv, asc);
      case 'p_win': av = a._norm.p_win; bv = b._norm.p_win; return compareNullable(av, bv, asc);
      case 'p_top3': av = a._norm.p_top3; bv = b._norm.p_top3; return compareNullable(av, bv, asc);
      case 'tansho_odds': av = a._norm.tansho_odds; bv = b._norm.tansho_odds; return compareNullable(av, bv, asc);
      case 'hole_score': av = raToNum(a._analysis?.hole_score); bv = raToNum(b._analysis?.hole_score); return compareNullable(av, bv, asc);
      case 'danger_score': av = raToNum(a._analysis?.danger_score); bv = raToNum(b._analysis?.danger_score); return compareNullable(av, bv, asc);
      case 'agreement_score': av = raToNum(a._analysis?.agreement_score); bv = raToNum(b._analysis?.agreement_score); return compareNullable(av, bv, asc);
      default:
        return compareNullable(a._norm.pred_order, b._norm.pred_order, true)
          || compareNullable(a._norm.p_top3, b._norm.p_top3, false);
    }
  }
